    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-[1200px] mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
            <MdOutlineEmail className="text-amber-500" /> Messages
          </h1>
          <p className="text-slate-500 text-sm mt-1">
            Communicate directly with {user?.role === 'developer' ? 'QA engineers' : 'developers'} on the platform
          </p>
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <h2 className="text-lg font-black text-slate-800">New Message</h2>
            ...
            <form onSubmit={handleSend} className="p-6 space-y-4">
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">
                  To ({user?.role === 'developer' ? 'QA Engineer' : 'Developer'}) *
                </label>
                <select required value={compose.receiver_id} onChange={e => setCompose(c => ({ ...c, receiver_id: e.target.value }))}
                  className="mt-1 w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/30">
                  <option value="">— Select recipient —</option>
                  {teamUsers.map(u => (
                    <option key={u.id} value={u.id}>{u.username} ({u.email})</option>
                  ))}
                  {teamUsers.length === 0 && <option disabled>No team members found</option>}
                </select>
              </div>
              <div>
                ...
                <input value={compose.subject} onChange={e => setCompose(c => ({ ...c, subject: e.target.value }))}
                  placeholder="e.g. Login button failing — test case TC-005"
                  className="mt-1 w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/30"
                />
              </div>
              {projects.length > 0 && (
                <div>
                  <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Link Project (optional)</label>
              ...
              <div>
                ...
                <textarea required value={compose.body} onChange={e => setCompose(c => ({ ...c, body: e.target.value }))}
                  placeholder="Describe the issue, question or update…"
                  rows={5}
                  className="mt-1 w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-amber-400/30"
                />
              </div>
              <div className="flex gap-3">
                ...
                <button type="submit" disabled={composeSending}
                  className="flex-1 py-2.5 bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-white rounded-xl text-sm font-bold flex items-center justify-center gap-2">
      ...
      {loading ? (
        ...
        <div className="bg-white border-2 border-dashed border-slate-200 rounded-3xl p-16 text-center">
          ...
          <p className="text-lg font-bold text-slate-700">{box === 'inbox' ? 'Inbox is empty' : 'No sent messages'}</p>
          <p className="text-slate-400 text-sm mt-1">
            {box === 'inbox'
              ? 'You have no messages. QA engineers can send you issues directly here.'
              : 'Click Compose to send a message to a team member.'}
          </p>
        </div>
      ) : (
          ...
          {messages.map(msg => (
            <div key={msg.id}
              className={`bg-white border rounded-2xl shadow-sm overflow-hidden transition-all ${
                !msg.is_read && box === 'inbox' ? 'border-amber-300 shadow-amber-100' : 'border-slate-200'
              }`}>
              {/* Message row */}
              <div
              ...
              >
                {!msg.is_read && box === 'inbox' ? (
                  <MdCircle className="text-amber-500 text-xs shrink-0 mt-1.5" />
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    ...
                    {msg.replies?.length > 0 && <span className="ml-2 text-violet-500 font-bold">• {msg.replies.length} {msg.replies.length === 1 ? 'reply' : 'replies'}</span>}
                  </p>
                ...
                <div className="border-t border-slate-100 bg-slate-50/50">
                  ...
                  <div className="px-6 py-4">
                    <div className="flex items-center gap-2 mb-3">
                      <div className="w-7 h-7 rounded-full bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white text-[10px] font-black">
                  ...
                  {msg.replies?.map((r, i) => (
                    <div key={i} className="px-6 py-3 border-t border-slate-100">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="w-6 h-6 rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-white text-[9px] font-black">
                      ...
                      <div className="ml-8 bg-violet-50 border border-violet-100 rounded-xl p-3">
                  ...
                  <div className="px-6 py-4 border-t border-slate-100">
                    <div className="flex gap-3">
                      <textarea
                        ...
                        rows={2}
                        className="flex-1 bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-amber-400/30"
                      />
                      <button
                        onClick={() => handleReply(msg.id)}
                        disabled={replySending === msg.id || !(replyText[msg.id] || '').trim()}
                        className="bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-white px-4 rounded-xl flex items-center gap-2 transition-colors text-sm font-bold shrink-0">
